...
{	  
  ...
  unsigned int offset = x + (xDim * y) + (xDim * yDim * z);
   ...
   if (x != 0){
	   ...
	   //linear penalty to gfunc if the neighbor's distance to its mean is beyond n standard deviations
	   if ( distance_to_mean > ( ndevs  * norm_2_sdevs[neigh_label]) ){
	   		
	   			gfunc = gfunc * ( (ndevs * norm_2_sdevs [neigh_label]) / (distance_to_mean ) );
