// compile with: nvcc growcut_3D_euclid_4_channels_exponential_penalty.cu -arch sm_20 -ptx 
...
// http://stackoverflow.com/questions/7318002/3d-image-indices
...
{	  
  ...
  {return;}
   ...
   float distance_to_mean = 0.0;

   float x_distance       = 0.0;
   
   float neigh_norm_2     = 0.0;
   ...
   if (x != 0){
	   ...
	   neigh_norm_2 = sqrt ( pow(d_channelOne  [offset], two ) 
	   											     +
	   ...
	   distance_to_mean = abs ( neigh_norm_2 - norm_2_means[neigh_label]);

	   //exponential penalty to gfunc if the neighbor's distance to its mean is beyond n standard deviations
	   if ( distance_to_mean > (ndevs * norm_2_sdevs[neigh_label]) ) {

                gfunc = gfunc * exp ( -1.0 * (distance_to_mean - ( ndevs * norm_2_sdevs[neigh_label]) ) );  
