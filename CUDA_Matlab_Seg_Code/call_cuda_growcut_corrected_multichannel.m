function [ segmentation, strength_map, steps, execution_time ] =...
    ...
    call_cuda_growcut_corrected_multichannel ( channelOne,...
                                               channelTwo,...
                                               channelThree,...
                                               channelFour,...
                                               label, strength, max_I )

% get gpu handle + specs and clear its existing memory
g = gpuDevice(1);
...
evol_cells_kernel      = parallel.gpu.CUDAKernel('growcut_3D_euclid_4_channels_corrected.ptx',...
                                                 'growcut_3D_euclid_4_channels_corrected.cu',...
                                                 '_Z10evol_cellsPKfS0_S0_S0_S0_PKhPfPhS4_iiif');

update_old_step_kernel = parallel.gpu.CUDAKernel('growcut_3D_euclid_4_channels_corrected.ptx',...
                                                 'growcut_3D_euclid_4_channels_corrected.cu',...
...
%count cells without label
% cells_without_label = (xDim*yDim*zDim) - nnz(d_new_label);

segmentation = gather(d_new_label);
strength_map = gather(d_new_strength);

clear d_new_label d_new_strength d_label d_strength d_channelOne...
      d_channelTwo d_channelThree d_channelFour
